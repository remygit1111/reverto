...
- `make test` passes (2093 expected; some test counts may shift slightly with conftest changes — must be explained).
- `make lint` clean.
...
| R12 | Plugin's own tests can't be open-sourced (would reveal closed-source code paths). | 3 | Medium | Low | Plugin CI is on a private GitHub repo. Plugin docs ship public, plugin tests stay private. |
