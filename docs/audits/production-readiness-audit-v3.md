...
### SHOULD-FIX (strongly recommended within 1-2 weeks)

5 items, ordered by impact:

1. **r3-001** (HIGH) — chart.py `{str(e)[:200]}` exception leak in 3 sites. ~10 min fix; mirror pd-001 template (`logger.exception` + generic detail). Same class-of-issue as pd-001/r2-001.
2. **r3-003** (MEDIUM) — Strip `Server` response header. 3 lines in `SecurityHeadersMiddleware`. ~5 min.
3. **r3-002** (MEDIUM) — Broaden `tests/test_response_body_hygiene.py` regex to catch function-wrapped exceptions (`{str(e)[:N]}`, `{repr(e)}`, etc.). Lands with r3-001 fix. ~15 min.
4. **r3-004** (LOW) — `usermod -a -G bot caddy` (operator-side, 1 command). Fixes maintenance-page 403. ~2 min.
5. **r3-005** (MEDIUM) — Make `uvicorn.Config(proxy_headers=True, forwarded_allow_ips="127.0.0.1")` explicit OR add HSTS `curl` check to runbook. ~5 min either way.
...
⚠️ **APPROVED WITH CONDITIONS** for continued public operation.

### Conditions

1. Land **r3-001** (chart.py exception leak) within 1 week. The pattern is identical to closed pd-001/r2-001 issues, the regression-test gap is the proximate cause, and ccxt error strings on a public domain are an unnecessary fingerprinting surface.
2. Land **r3-002** (broaden regex) WITH r3-001 fix. Without it, the fix could be reverted unnoticed.
3. Land **r3-003** (Server header) within 1 week. 5-minute change with material info-leak reduction.
