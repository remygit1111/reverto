...
class TestMasterKey:
    def test_key_file_auto_created_on_first_use(self, tmp_store):
        ...
        credentials.save_keys("bitget", "ak", "sc")
    ...
    def test_key_file_reused_across_calls(self, tmp_store):
        ...
        credentials.save_keys("kraken", "ak2", "sc2")
...
class TestSaveAndGet:
    def test_roundtrip(self, tmp_store):
        credentials.save_keys("bitget", "my-api-key", "my-secret")
        got = credentials.get_keys("bitget")
        assert got == {"api_key": "my-api-key", "api_secret": "my-secret"}

    def test_get_unknown_returns_none(self, tmp_store):
        assert credentials.get_keys("bitget") is None

    def test_multiple_exchanges(self, tmp_store):
        credentials.save_keys("bitget", "b-key", "b-sec")
        credentials.save_keys("kraken", "k-key", "k-sec")
        assert credentials.get_keys("bitget") == {
            "api_key": "b-key", "api_secret": "b-sec",
        }
        assert credentials.get_keys("kraken") == {
            "api_key": "k-key", "api_secret": "k-sec",
        }

    def test_ciphertext_is_actually_encrypted(self, tmp_store):
        credentials.save_keys("bitget", "plain-text-key", "plain-text-secret")
...
class TestHasKeys:
    def test_false_when_empty(self, tmp_store):
        assert credentials.has_keys("bitget") is False

    def test_true_after_save(self, tmp_store):
        credentials.save_keys("bitget", "ak", "sc")
        assert credentials.has_keys("bitget") is True
        assert credentials.has_keys("kraken") is False


class TestDeleteKeys:
    def test_delete_existing(self, tmp_store):
        credentials.save_keys("bitget", "ak", "sc")
        assert credentials.delete_keys("bitget") is True
        assert credentials.get_keys("bitget") is None
        assert credentials.has_keys("bitget") is False

    def test_delete_unknown_returns_false(self, tmp_store):
        assert credentials.delete_keys("bitget") is False

    def test_delete_leaves_other_entries(self, tmp_store):
        credentials.save_keys("bitget", "b", "B")
        credentials.save_keys("kraken", "k", "K")
        credentials.delete_keys("bitget")
        assert credentials.get_keys("bitget") is None
        assert credentials.get_keys("kraken") == {"api_key": "k", "api_secret": "K"}


class TestListExchanges:
    def test_empty(self, tmp_store):
        assert credentials.list_exchanges_with_keys() == []

    def test_after_save(self, tmp_store):
        credentials.save_keys("kraken", "k", "K")
        credentials.save_keys("bitget", "b", "B")
        # Sorted alphabetically
        assert credentials.list_exchanges_with_keys() == ["bitget", "kraken"]

    def test_after_delete(self, tmp_store):
        credentials.save_keys("bitget", "b", "B")
        credentials.save_keys("kraken", "k", "K")
        credentials.delete_keys("bitget")
        assert credentials.list_exchanges_with_keys() == ["kraken"]
...
class TestDecryptFailure:
    def test_tampered_ciphertext_returns_none(self, tmp_store):
        ...
        result = credentials.get_keys("bitget")
