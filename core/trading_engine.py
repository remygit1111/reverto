...
# Abstract base class for trading engines (Phase 2 Task 2.1).
# Extracted mechanically from paper/paper_engine.py — same code,
# new location. Holds the shared trading + infrastructure semantics
# used by both paper and live modes. Subclasses provide the mode-
# specific _deduct_balance implementation.
...
class TradingEngine(ABC):
    ...
    Defines the shared trading + infrastructure semantics used by
    both paper and live trading modes. Subclasses provide mode-
    specific implementations for:
    - _deduct_balance: balance enforcement (simulated vs real)
    - _place_market_order: order execution (simulated vs exchange)
    - _get_current_price: price source (polled ticker vs realtime)
    """
    ...
    def _deduct_balance(self, amount: float, reason: str) -> bool:
        """Subclass-specific balance check.

        PaperEngine simulates an insufficient-funds check against
        an in-memory balance. LiveEngine queries the exchange's
        reported balance via pre-flight check.
        """
        ...
